  ...
  Object.keys(src).forEach((recId) => {
    ...
    rows.forEach((row, idx) => {
      ...
      out.push({
        ...
        contingency: '',
        status: p.management ? 'inprogress' : 'open',
        residual: '',
        dueDate: '',
        reviewDate: '',
      ...
      return `<div class="rrg-factor" onclick="rrgOpenDetail(${r.id})">
        ...
        <div class="rrg-factor-right" onclick="event.stopPropagation()">
          ...
          <div class="rrg-factor-actions">
            ...
            ${canEdit && r.source==='prm' ? `<button class="btn btn-sm" onclick="showPage('risk')" style="color:var(--text2);padding:3px 6px;font-size:11px" title="แก้ไขที่แผนบริหารความเสี่ยง (PRM)">🗂️ ไปที่ PRM</button>` : ''}
          </div>
        </div>
      </div>`;
    }).join('');
  ...
  tbody.innerHTML = page.map((r, idx) => {
    ...
    return `<tr onclick="rrgOpenDetail(${r.id})" style="cursor:pointer">
      ...
      <td class="rrg-td-actions" onclick="event.stopPropagation()">
        ...
        ` : ''}
        ${canEdit && r.source==='prm' ? `
        <button class="btn btn-sm" onclick="showPage('risk')" style="color:var(--text2);padding:4px 8px;font-size:11px" title="แก้ไขที่แผนบริหารความเสี่ยง (PRM)">🗂️ ไปที่ PRM</button>
        ` : ''}
      </td>
    </tr>`;
  }).join('');
...
function rrgOpenDetail(id) {
  ...
  // edit button only for members และไม่แสดงสำหรับรายการที่ดึงมาจาก PRM (ให้ไปแก้ที่หน้า PRM แทน)
...
async function rrgExportProjectPDF(encProj) {
  ...
  try {
    ...
    await _runPdfExport(html, 'riskregister-70-project-' + safeName, _rrgPdfHeaderOpts('รายงานรายโครงการ (แผน + ผลการดำเนินการ)'));
  } catch(err) {
    console.error(err);
    showToast('❌ สร้าง PDF ไม่สำเร็จ: ' + err.message);
  }
...
function _rrgBuildProjectHTML(proj) {
  ...
  const rows = items.map((r, i) => {
    ...
    return `<tr style="border-bottom:1px solid #e4e7ed">
      ...
      <td style="padding:6px;font-size:10.5px">${r.name||'—'}<br><span style="font-size:9.5px;color:#5a6477">${RRG_CAT_NAMES[r.category]||r.category||''}${r.source==='prm'?' · 📋 PRM':''}</span></td>
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid #e4e7ed">
      <thead>
        <tr style="background:#dc2626;color:#fff">
          ...
          <th style="padding:7px 6px;font-size:10.5px;text-align:left">ปัจจัยความเสี่ยง / ประเภท</th>
