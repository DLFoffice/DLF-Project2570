  ...
  Object.keys(src).forEach((recId) => {
    ...
    rows.forEach((row, idx) => {
      ...
      out.push({
        ...
        note: [p.timeline ? ('ระยะเวลา: ' + p.timeline) : '', p.budget ? ('งบประมาณ: ' + (typeof formatMoneyText === 'function' ? formatMoneyText(p.budget) : p.budget) + ' บาท') : '', p.reportNote ? ('รายงานผล: ' + p.reportNote) : '']
                .filter(Boolean).join(' · '),
